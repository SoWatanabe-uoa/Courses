...
GLboolean g_show_connected_components = 0;
...
void display(void)
{
    ...
    if (g_flat && !g_show_connected_components) {
    ...
    else if (!g_show_connected_components) {
        ...
        for (i = 0; i < nt; ++i) {
            Vector3 p[3]; getTriangleVertices(&g_tri_mesh, i, p);
            Triple tri = g_tri_mesh._triangles[i];
            float c0[3]; getComponentColor(&g_tri_mesh, tri._v0, c0);
            float c1[3]; getComponentColor(&g_tri_mesh, tri._v1, c1);
            float c2[3]; getComponentColor(&g_tri_mesh, tri._v2, c2);
            glBegin(GL_TRIANGLES);
...
void handleKeyEvents(unsigned char key, int x, int y)
{
    ...
    case 'c':
        computeConnectedComponents(&g_tri_mesh);
        g_show_connected_components = 1 - g_show_connected_components;
        glutPostRedisplay();
        break;
