...
GLfloat innerProd(GLfloat* a, GLfloat* b){
  return a[0]*b[0] + a[1]*b[1] + a[2]*b[2];
}

void normalize(GLfloat* u) {
  GLfloat umag = sqrt(u[0]*u[0]+u[1]*u[1]+u[2]*u[2]);
  if(fabs(umag) < 1e-5f) umag= 1.f;
  u[0] = u[0] / umag; u[1] = u[1] / umag; u[2] = u[2] / umag;
}

// Obtain an unit vector from current point to the light source
//
// Input:
// cp is a 3x1 vector which points at a current vertex position
// num_light is the number of light we deal with currently
//
// Output:
// L is a 3x1 unit vector
//
GLfloat getL(GLfloat* cp, int num_light, GLfloat* L){
  for(int i = 0; i < 3; ++i){
    L[i] = g_light_pos[num_light][i] - cp[i];
  }
  normalize(L);
}

// Obtain an unit vector from current point to the view point
//
// Input:
// cp is a 3x1 vector which points at a current vertex position
//
// Output:
// V is a 3x1 unit vector
//
GLfloat getV(GLfloat* cp, GLfloat* V){
  GLfloat v[3] = {0.0, 0.0, 10.0}; //view point
  for(int i = 0; i < 3; ++i){
    V[i] = /*v[i]*/ - cp[i];
  }
  normalize(V);
...
// Obtain an unit reflected vector
//
// Input:
// L is an unit vector from current point to the light source
// N is is the surface normal at current point
//
// Output:
// R is a 3x1 unit vector
//
GLfloat getR(GLfloat* L, GLfloat* N, GLfloat* R){
  GLfloat I[3] = {-L[0], -L[1], -L[2]}; // Incident vector
  for(int i = 0; i < 3; ++i){
    R[i] = I[i] - 2.f * N[i] * innerProd(I,N);
  }
}

// Obtain an half vector

GLfloat geth(GLfloat* V, GLfloat* L, GLfloat* h){
  for(int i = 0; i < 3; ++i){
    h[i] = V[i] + L[i];
  }
  normalize(h);
...
static void PhongModel(GLfloat* pe, GLfloat* ne, GLfloat* out_color) {
	...
	// Unit vector from current point to the light source
	GLfloat L[3];
	// Unit view-point vector
	GLfloat V[3];
	...
	for(i = 0; i < g_num_lights; ++i){
	  ...
	  out_color[0] = g_mat_amb[0] * g_light_amb[i][0]
	    + g_mat_diff[0] * g_light_diff[i][0] * fmaxf(innerProd(ne,L), 0.f);

	  out_color[1] = g_mat_amb[1] * g_light_amb[i][1]
	    + g_mat_diff[1] * g_light_diff[i][1] * fmaxf(innerProd(ne,L), 0.f);

	  out_color[2] = g_mat_amb[2] * g_light_amb[i][2]
	    + g_mat_diff[2] * g_light_diff[i][2] * fmaxf(innerProd(ne,L), 0.f);
	  
	  if(phong_blinn){
	    //out_color[0] += g_mat_spec[0] * g_light_spec[i][0] * powf(fmaxf(innerProd(V,R), 0.f), g_mat_shiny);
	    //out_color[1] += g_mat_spec[1] * g_light_spec[i][1] * powf(fmaxf(innerProd(V,R), 0.f), g_mat_shiny);
	    //out_color[2] += g_mat_spec[2] * g_light_spec[i][2] * powf(fmaxf(innerProd(V,R), 0.f), g_mat_shiny);
	  }
	  else{
	    //out_color[0] += g_mat_spec[0] * g_light_spec[i][0] * powf(fmaxf(innerProd(h,ne), 0.f), g_mat_shiny);
	    //out_color[1] += g_mat_spec[1] * g_light_spec[i][1] * powf(fmaxf(innerProd(h,ne), 0.f), g_mat_shiny);
	    //out_color[2] += g_mat_spec[2] * g_light_spec[i][2] * powf(fmaxf(innerProd(h,ne), 0.f), g_mat_shiny);
	  }
	}
